...
class TradingViewPage(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        row_quote = QHBoxLayout()
        row_quote.addWidget(QLabel("期貨 Ratio/Offset 報價源："))
        self.cb_futures_quote_source = QComboBox()
        self.cb_futures_quote_source.addItem("yfinance（US RTH 內準確；盤外會 skip）", "yfinance")
        self.cb_futures_quote_source.addItem("tv_legend（24/5；盤外用，會短暫開 tab）", "tv_legend")
        self.cb_futures_quote_source.setToolTip(
            "ES1!/NQ1!/RTY1! 子圖的 TO FUTURE 欄位用哪個來源算 ratio/offset。\n"
            "yfinance：直接 Python 抓 Yahoo；^GSPC/QQQ/IWM 只在 US RTH 更新，盤外會 freshness 失敗。\n"
            "tv_legend：在 CDP 內開暫存 tab 讀 FOREXCOM:SPX500 等 24/5 即時價，盤外也準。"
        )
        self.cb_futures_quote_source.currentIndexChanged.connect(self._save_tv_prefs)
        row_quote.addWidget(self.cb_futures_quote_source)
        row_quote.addStretch(1)
        go.addLayout(row_quote)
    ...
    def _load_tv_prefs(self) -> None:
        ...
        quote_src = str(cfg.get("futures_quote_source") or "yfinance").strip() or "yfinance"
        q_idx = self.cb_futures_quote_source.findData(quote_src)
        if q_idx >= 0:
            self.cb_futures_quote_source.setCurrentIndex(q_idx)
    ...
    async def _maybe_fill_to_future(
        self,
        *,
        automator: PlaywrightCDPAutomator,
        chosen: str | None,
        ...
        """Fill TO FUTURE Ratio/Offset for today's column when rule matches.

        Returns True iff a DOM write was attempted on TO FUTURE (so the caller
        knows to save=True even when the WEEKLY GEX LEVELS section was already
        full). Returns False for all skip/preview/error paths (no write).

        Strict guards (all must hold before any DOM read/write):
          1. (futures_root, layout_mode) is in quote_source.RULES
          2. row_monday equals the current local week's Monday
          3. today is Mon~Fri (weekend → skip with log)
          4. Daily <kind> and today's <kind> are both at TV default

        Assumes the indicator-properties-dialog is currently open (called from
        on_target_row, after fill_weekly_levels and before close_settings).
        """
        futures_root = self._futures_root_from_symbol(chosen)
        if not futures_root:
            return False
        rule_entry = _quote_source.RULES.get((futures_root, layout_mode))
        if rule_entry is None:
            return False
        rule, kind = rule_entry
        ...
        if weekday >= 5:
            self._log_event(
                "skip",
                "略過｜週末",
                f"{futures_root} [{layout_mode}] {kind} 不填（今日週末）",
                layout=layout_name,
                subchart=sub_index,
                ticker=target_ticker,
                detail=(
                    f"URL={url_for_log}\n圖上={chosen}\nticker={target_ticker}\n"
                    f"原因：今日為週末，TO FUTURE 不自動填入"
                ),
            )
            return False
        current_monday = now.date() - timedelta(days=weekday)
        if row_monday != current_monday:
            # 非當週的 row：靜默 skip（避免每個 row 都刷一條 log）
            return False

        day_name = _DAY_ORDER[weekday]

        # Dry-run preview: skip DOM read, just announce and exit.
        if opts.dry_run:
            self._log_event(
                "preview",
                "預覽｜TO FUTURE",
                f"{futures_root} [{layout_mode}] {day_name} {kind} 將以 {opts.futures_quote_source} 報價填入",
                layout=layout_name,
                subchart=sub_index,
                ticker=target_ticker,
                detail=(
                    f"URL={url_for_log}\n圖上={chosen}\nticker={target_ticker}\n"
                    f"規則：{rule.futures_tv} vs {rule.compare_tv}（op={rule.op}）"
                ),
            )
            return False

        # Skip if Daily or today's field is already non-default.
        try:
            daily_val = await automator.read_to_future_value("Daily", kind)
            day_val = await automator.read_to_future_value(day_name, kind)
        except Exception as exc:  # noqa: BLE001 - 讀取失敗就 skip，不影響 WEEKLY GEX LEVELS
            self._log_event(
                "error",
                "略過｜TO FUTURE 讀取失敗",
                f"{futures_root} [{layout_mode}] {kind}: {exc}",
                layout=layout_name,
                subchart=sub_index,
                ticker=target_ticker,
                detail=(
        ...
        if _quote_source.is_nondefault(daily_val, kind) or _quote_source.is_nondefault(day_val, kind):
            self._log_event(
                "skip",
                "略過｜TO FUTURE 已有值",
                f"{futures_root} [{layout_mode}] {day_name} {kind} 已被改過（Daily={daily_val!r} / {day_name}={day_val!r}）",
                layout=layout_name,
                subchart=sub_index,
                ticker=target_ticker,
                detail=(
                    f"URL={url_for_log}\n圖上={chosen}\nticker={target_ticker}\n"
                    f"原因：Daily 或 {day_name} {kind} 已偏離 default（Ratio=1 / Offset=0），不覆蓋"
                ),
            )
            return False
        ...
        # Fetch quote.
        try:
            result = await _quote_source.fetch_value(
                rule,
                opts.futures_quote_source,  # type: ignore[arg-type]
                automator=automator,
            )
        except Exception as exc:  # noqa: BLE001
            self._log_event(
                ...
                detail=(
                    f"URL={url_for_log}\n圖上={chosen}\nticker={target_ticker}\n"
                    f"來源={opts.futures_quote_source}\n錯誤：{exc}"
                ),
            )
            return False
        ...
        if result.value is None:
            self._log_event(
                "error",
                "略過｜TO FUTURE 報價失敗",
                f"{futures_root} [{layout_mode}] {kind}: {result.reason or '無報價'}",
                layout=layout_name,
                subchart=sub_index,
                ticker=target_ticker,
                detail=(
                    f"URL={url_for_log}\n圖上={chosen}\nticker={target_ticker}\n"
                    f"來源={opts.futures_quote_source}\n原因：{result.reason or '無報價'}"
                ),
            )
            return False
        value = result.value

        # If computed value rounds to default, skip writing (avoid trivial diffs).
        default_val = 1.0 if kind == "Ratio" else 0.0
        if abs(value - default_val) < 1e-6:
            self._log_event(
                "info",
                "略過｜TO FUTURE 等於 default",
                f"{futures_root} [{layout_mode}] {day_name} {kind}={value:.6f} ≈ default",
                layout=layout_name,
                subchart=sub_index,
                ticker=target_ticker,
                detail=(
                    f"URL={url_for_log}\n圖上={chosen}\nticker={target_ticker}\n"
                    f"原因：當下值與 default 差距 <1e-6，無須寫入"
                ),
            )
            return False

        formatted = _quote_source.format_value(value, kind)
        try:
            ok = await automator.fill_to_future_value(day_name, kind, formatted)
        except Exception as exc:  # noqa: BLE001
            self._log_event(
                "error",
                "失敗｜TO FUTURE 寫入",
                f"{futures_root} [{layout_mode}] {day_name} {kind}={formatted}: {exc}",
                layout=layout_name,
                subchart=sub_index,
                ticker=target_ticker,
                detail=(
        ...
        status_tag = "done" if ok else "warn"
        msg_extra = "" if ok else "（讀回值不符，但已寫入）"
        self._log_event(
            status_tag,
            "更新 TO FUTURE",
            f"{futures_root} [{layout_mode}] {day_name} {kind}={formatted}{msg_extra}",
            layout=layout_name,
            subchart=sub_index,
            ticker=target_ticker,
            detail=(
                f"URL={url_for_log}\n圖上={chosen}\nticker={target_ticker}\n"
                f"來源={opts.futures_quote_source}  {rule.futures_yf} vs {rule.compare_yf}（op={rule.op}）"
            ),
        )
        return True
    ...
    @staticmethod
    def _futures_root_from_symbol(symbol: str | None) -> str | None:
        """Return the futures root (tail after ':'), preserving '!' suffix.

        E.g. ``CME_MINI:NQ1!`` → ``NQ1!``. Used to key into ``quote_source.RULES``.
        """
        if not symbol:
            return None
        tail = str(symbol).split(":", 1)[-1].strip().upper()
        return tail or None
