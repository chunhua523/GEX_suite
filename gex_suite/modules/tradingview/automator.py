...
class PlaywrightCDPAutomator(TVAutomator):
    ...
    async def list_layouts(self) -> list[LayoutInfo]:
        ...
        Primary path: stamp the dialog's inner scroll container and step
        ``scrollTop`` in JS, harvesting rows each iteration. Dedup by chart
        URL when available — TradingView allows duplicate (name, subtitle)
        pairs (e.g. user keeps two ``MSFT`` layouts), so name-based dedup
        silently drops real entries.

        Fallback path (when no scroll container can be identified): the
        legacy mouse-wheel scroll loop, kept verbatim for resilience against
        TV DOM reshuffles.
        """
        page = self._require_page()
        await page.bring_to_front()
        ...
        out: list[LayoutInfo] = []
        # Dedup tracks both URL and name|subtitle keys: URL is canonical when
        # available (catches same-name duplicates); textual key prevents the
        # same render from being appended twice when URL extraction fails.
        seen_url_keys: set[str] = set()
        seen_text_keys: set[str] = set()
        first_row_dumped_box = [False]

        container_info = await self._stamp_layout_scroll_container(dialog)
        used_js_scroll = False

        if container_info is not None:
            used_js_scroll = True
            sh = int(container_info.get("sh", 0))
            ch = max(1, int(container_info.get("ch", 1)))
            # Reset to top, sweep downward in 75% clientHeight increments.
            await self._js_set_scroll(0)
            await page.wait_for_timeout(180)
            await self._harvest_layout_rows_batched(
                out, seen_url_keys, seen_text_keys
            )

            step = max(60, int(ch * 0.5))
            iters = 0
            max_iters = max(40, (sh // step) + 8)
            stagnant_rounds = 0
            last_total = -1
            while iters < max_iters:
                iters += 1
                moved = await self._js_scroll_by(step)
                ...
                if moved is None:
                    used_js_scroll = False
                    break
                total = len(seen_url_keys) + len(seen_text_keys)
                if total == last_total:
                    stagnant_rounds += 1
                else:
                    stagnant_rounds = 0
                    last_total = total
                if moved["after"] + moved["ch"] >= moved["sh"] - 2:
                    break
                if stagnant_rounds >= 6:
                    break

            # Final bottom-pin + double harvest to catch trailing rows that
            # virtualization had not yet rendered when the sweep terminated.
            await self._js_set_scroll(-1)
        ...
        if not used_js_scroll:
            # Legacy wheel-scroll fallback. Slower and incomplete on long
            # lists, but resilient when JS can't identify the inner scroller.
            stagnant_rounds = 0
            last_total = -1
            for _ in range(14):
                await self._harvest_layout_rows(
                    rows, out, seen_url_keys, seen_text_keys, first_row_dumped_box
                )
                total = len(seen_url_keys) + len(seen_text_keys)
                if total == last_total:
                    stagnant_rounds += 1
        ...
        await self._clear_layout_scroll_stamp()
        ...
        path = "js_scroll" if used_js_scroll else "wheel_fallback"
        self._log(f"[list_layouts] total={len(out)} with_url={with_url} path={path}")
        await self._close_layout_dialog_gently()
        return out or [LayoutInfo(id="current", name="Current")]

    async def _harvest_layout_rows(
        self,
        rows,
        out: list[LayoutInfo],
        seen_url_keys: set[str],
        seen_text_keys: set[str],
        first_row_dumped_box: list,
    ) -> None:
        """Read all currently-DOM-present rows and append new ones to ``out``.

        Dedup priority:
          1. URL (chart id) — canonical, distinguishes same-name layouts.
          2. ``name|subtitle`` — used only when URL is missing for this row.
        """
        ...
        for idx in range(count):
            ...
            if url:
                ...
                if text_key in seen_text_keys:
                    continue
            if not first_row_dumped_box[0] and url is None:
                await self._dump_row_outer_html(row, label="layout_row_first")
                first_row_dumped_box[0] = True
            if url:
    ...
    async def _harvest_layout_rows_batched(
        self,
        out: list[LayoutInfo],
        seen_url_keys: set[str],
        seen_text_keys: set[str],
    ) -> None:
        """Batched harvest: one ``page.evaluate`` round-trip extracts text +
        href for every DOM-present row, then Python dedups against caller
        sets.

        Roughly 10x faster than :meth:`_harvest_layout_rows` on long lists
        because the per-row Playwright traversal (inner_text, attribute
        lookups, URL fallbacks) all happens inside the page context.
        """
        page = self._require_page()
        try:
            rows_data = await page.evaluate(
                """
                (attr) => {
                  const container = document.querySelector('[' + attr + ']');
                  if (!container) return [];
                  // Match the same row set the Python locator targets. Order
                  // matters: more specific selectors first.
                  const rowSelector = (
                  ...
                  const normalizeUrl = (u) => {
                    ...
                    if (u.startsWith('/')) return 'https://www.tradingview.com' + u;
                    return '';
                  };
                  const out = [];
                  for (const r of rows) {
                    ...
                    // URL extraction: anchor href → row attribute → data-id.
                    let url = '';
                    const anchor = r.querySelector("a[href*='/chart/'], a[href]");
                    if (anchor) url = normalizeUrl(anchor.getAttribute('href') || '');
                    if (!url) {
                      for (const a of ['data-href','href','data-url','data-target-url']) {
                        const v = r.getAttribute(a);
                        if (v) { url = normalizeUrl(v); if (url) break; }
                      }
                    }
                    if (!url) {
                      for (const a of ['data-layout-id','data-id','data-saved-chart-id']) {
                        const v = r.getAttribute(a);
                        if (v && /^[A-Za-z0-9_-]{4,}$/.test(v)) {
                          url = 'https://www.tradingview.com/chart/' + v + '/';
                          break;
                        }
                      }
                    }
        ...
        for rd in rows_data:
            ...
            if not name:
                continue
            subtitle = (rd.get("subtitle") or "").strip() or None
            url = (rd.get("url") or "").strip() or None
            text_key = f"{name}|{subtitle}" if subtitle else name
            if url:
                if url in seen_url_keys:
                    continue
                seen_url_keys.add(url)
    ...
    async def _stamp_layout_scroll_container(self, dialog) -> dict | None:
        """Identify dialog's inner scrollable element and stamp it.

        Returns ``{sh, ch}`` of the chosen container, or ``None`` if no
        element with ``overflow-y: auto/scroll`` and meaningful scroll depth
        was found (in which case the caller falls back to wheel scrolling).
        """
        page = self._require_page()
        try:
        ...
        try:
            info = await page.evaluate(
                ...
                ({root, attr}) => {
                  ...
                  return { sh: best.sh, ch: best.ch };
                }
                """,
                {"root": handle, "attr": self._LAYOUT_SCROLL_ATTR},
            )
        except Exception:
            info = None
        if not info:
    ...
    async def _js_set_scroll(self, top: float) -> dict | None:
        page = self._require_page()
        try:
            return await page.evaluate(
                ...
                ({attr, top}) => {
                  ...
                  if (!el) return null;
                  if (top < 0) top = el.scrollHeight;
                  el.scrollTop = top;
                  return { after: el.scrollTop, sh: el.scrollHeight, ch: el.clientHeight };
                }
                """,
                {"attr": self._LAYOUT_SCROLL_ATTR, "top": top},
            )
        except Exception:
            return None

    async def _js_scroll_by(self, delta: float) -> dict | None:
        page = self._require_page()
        try:
            return await page.evaluate(
                """
                ({attr, delta}) => {
                  ...
                  return { before, after: el.scrollTop, sh: el.scrollHeight, ch: el.clientHeight };
                }
                """,
                {"attr": self._LAYOUT_SCROLL_ATTR, "delta": delta},
            )
        except Exception:
            return None

    async def _clear_layout_scroll_stamp(self) -> None:
        page = self._require_page()
        try:
            await page.evaluate(
                """
                  ...
                  const visibleRowCount = () =>
                    Array.from(
                      root.querySelectorAll(
                        "[data-name='legend-source-item'], [data-qa-id='legend-source-item'], [class*='sourceItem'], [class*='item'][class*='study']"
                      )
    ...
    async def get_active_symbol_candidates(self) -> list[str]:
        ...
        selectors = [
            "[data-name='legend-source-item-main-series'], [data-qa-id='legend-series-item']",
    ...
    async def list_chart_indicators(self) -> list[IndicatorInfo]:
        ...
        selectors = [
            ...
            "div[data-qa-id='legend'] [data-qa-id='legend-source-item']",
    ...
    async def open_indicator_settings(self, title_keyword: str = "Daily & Weekly GEX") -> None:
        ...
        gear_btn = indicator.locator(
            ...
            "[data-name*='legend-source-item-settings'], [data-qa-id='legend-settings-action'], "
        ...
        try:
            title = await locator.evaluate(
                ...
                (el) => {
                  ...
                  const named = [
                    row.querySelector("[data-name='legend-source-item-title'], [data-qa-id~='legend-source-title']"),
                    row.querySelector("[data-name*='legend-source-item-title'], [data-qa-id~='legend-source-title']"),
    ...
    async def count_indicators(self, title_keyword: str = "Daily & Weekly GEX") -> int:
        ...
        candidates = page.locator(
            f"[data-name='legend-source-item'], [data-qa-id='legend-source-item']:has-text('{_q(title_keyword)}'), "
    ...
    # ------------------------------------------------------------------
    # TO FUTURE section (Ratio/Offset for Daily + Monday~Friday)
    # ------------------------------------------------------------------
    # Mirrors the WEEKLY GEX LEVELS helpers above. The section contains 6 rows
    # (Daily + Mon~Fri), each row has two inputs labeled "<day> Ratio" and
    # "<day> Offset". Default values: Ratio=1, Offset=0.

    _TO_FUTURE_DAYS = ("Daily", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday")
    _TO_FUTURE_KINDS = ("Ratio", "Offset")

    async def _get_to_future_section(self):
        dialog = await self._get_indicator_properties_dialog()
        if dialog is None:
            return None
        for selector in (
            "xpath=.//*[@data-qa-id='property-dialog-item TO FUTURE']"
            "/ancestor::div[contains(@class,'titleWrap')][1]",
            "xpath=.//*[@data-qa-id='property-dialog-item To Future']"
            "/ancestor::div[contains(@class,'titleWrap')][1]",
            # Case-insensitive fallback: header text equals 'TO FUTURE'
            "xpath=.//*[translate(normalize-space(.),"
            "'abcdefghijklmnopqrstuvwxyz','ABCDEFGHIJKLMNOPQRSTUVWXYZ')='TO FUTURE']"
            "/ancestor::div[contains(@class,'titleWrap')][1]",
        ):
            section = dialog.locator(selector).first
            if await section.count():
                return section
        return None

    async def _find_to_future_input(self, section, day: str, kind: str):
        """Locate the input matching ``"<day> <kind>"`` label inside TO FUTURE.

        Scrolls the dialog a few times in case lower rows are lazy-rendered.
        """
        page = self._require_page()
        dialog = page.locator("[role='dialog'][data-name='indicator-properties-dialog']").last
        label = f"{day} {kind}"
        ...
        for _ in range(8):
            ...
            candidate = section.locator(
                ...
                f"{label_xpath}][1]/following::input[1]"
            ).first
            if await candidate.count():
                return candidate
            await dialog.hover()
    ...
    async def read_to_future_value(self, day: str, kind: str) -> str | None:
        """Read the current ``"<day> <kind>"`` value from TO FUTURE section.

        Caller must have an open indicator-properties-dialog (Inputs tab).
        Returns the raw string (may be empty); ``None`` if input not found.
        """
        if day not in self._TO_FUTURE_DAYS or kind not in self._TO_FUTURE_KINDS:
            raise ValueError(f"invalid TO FUTURE coord: day={day}, kind={kind}")
        await self._select_indicator_tab("Inputs")
        section = await self._get_to_future_section()
        if section is None:
            return None
        input_box = await self._find_to_future_input(section, day, kind)
        if input_box is None:
            return None
        try:
            return await input_box.input_value(timeout=600)
        except Exception:
            return None

    async def fill_to_future_value(self, day: str, kind: str, value: str) -> bool:
        """Write ``value`` to the ``"<day> <kind>"`` input in TO FUTURE section.

        Returns True if the input value reads back as ``value`` after write.
        Caller is responsible for saving the dialog (close_settings(save=True)).
        """
        if day not in self._TO_FUTURE_DAYS or kind not in self._TO_FUTURE_KINDS:
            raise ValueError(f"invalid TO FUTURE coord: day={day}, kind={kind}")
        page = self._require_page()
        ...
        section = await self._get_to_future_section()
        if section is None:
            await self._dump_dom("to_future_section_missing")
            raise RuntimeError("Could not find TO FUTURE section.")
        input_box = await self._find_to_future_input(section, day, kind)
        if input_box is None:
            await self._dump_dom(f"to_future_input_missing_{day.lower()}_{kind.lower()}")
            raise RuntimeError(f"Could not find {day} {kind} input in TO FUTURE.")
        await self._fill_input_value(input_box, value)
        await page.wait_for_timeout(80)
        try:
            actual = await input_box.input_value(timeout=600)
        except Exception:
