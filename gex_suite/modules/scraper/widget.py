...
class ScraperPage(QWidget):
    ...
    def _run_job_thread(
        ...
        try:
            ...
            self._salvage_failed_tasks_from_runner()
        finally:
    ...
    def _job_finished(self) -> None:
        ...
        self._persist_last_failed_tasks()
        self.current_log_file = None

    # ---------- last_failed_tasks persistence ----------
    # Shares last_result.json with gex_suite/api/routes/scraper.py — same shape:
    # {"last_result": ..., "failed_tasks": [...]}. Read-modify-write to avoid
    # clobbering the API's last_result key when both run against the same data dir.
    def _load_last_failed_tasks(self) -> None:
        if not SCRAPER_LAST_RESULT_PATH.exists():
            return
        try:
            with SCRAPER_LAST_RESULT_PATH.open("r", encoding="utf-8") as f:
                payload = json.load(f)
        except Exception as exc:
            print(f"[Scraper] failed to load last_result.json: {exc}")
            return
        failed = payload.get("failed_tasks") if isinstance(payload, dict) else None
        if isinstance(failed, list) and failed:
            self.last_failed_tasks = failed
            self.btn_retry.setEnabled(True)
            self._log(f"Loaded {len(failed)} previously failed items. RETRY FAILED is available.")

    def _persist_last_failed_tasks(self) -> None:
        ensure_dirs()
        existing: dict = {}
        if SCRAPER_LAST_RESULT_PATH.exists():
            try:
                with SCRAPER_LAST_RESULT_PATH.open("r", encoding="utf-8") as f:
                    loaded = json.load(f)
                ...
                print(f"[Scraper] failed to read last_result.json, rewriting: {exc}")
        existing["failed_tasks"] = self.last_failed_tasks
        try:
            with SCRAPER_LAST_RESULT_PATH.open("w", encoding="utf-8") as f:
                json.dump(existing, f, indent=2, ensure_ascii=False)
        except Exception as exc:
            print(f"[Scraper] failed to save last_result.json: {exc}")
    ...
    def _salvage_failed_tasks_from_runner(self) -> None:
        """When asyncio.run raises (e.g. STOP closes the browser mid-flight) the
        return-value assignment is skipped, leaving ``last_failed_tasks`` at its
        pre-run value. Recover the runner's recorded structured failures so the
        persisted list reflects what actually happened (re-failed + Stopped)."""
        runner = self.scraper_instance
        if runner is None:
            return
        salvaged = getattr(runner, "failed_tasks_structured", None)
        if isinstance(salvaged, list):
            self.last_failed_tasks = list(salvaged)
            self._log_safe(
                f"Recovered {len(self.last_failed_tasks)} failed items from interrupted job."
            )
