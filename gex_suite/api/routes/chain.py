...
def chain_start() -> ChainTriggerResponse:
    ...
    try:
        ...
        pass
    # Spawn via osascript "do shell script" so the chain inherits the user's
    # GUI session TCC (Full Disk Access). Without this wrapper, a direct
    # /bin/zsh subprocess can't read ~/Library/CloudStorage/... or ~/Documents/
    # — same trick the launchd plist uses for the scheduled path. New session
    # so /chain/stop can killpg the whole tree.
    try:
        proc = subprocess.Popen(
            ["/usr/bin/osascript", "-e",
             f'do shell script "{CHAIN_LAUNCH_NOW_SH}"'],
